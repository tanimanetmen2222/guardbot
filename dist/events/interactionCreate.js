...
function dbGet(sql, params = []) {
    return new Promise((resolve, reject) => {
        database_1.db.get(sql, params, (err, row) => {
            if (err)
                reject(err);
            else
                resolve(row);
...
function dbRun(sql, params = []) {
    return new Promise((resolve, reject) => {
        database_1.db.run(sql, params, (err) => {
            if (err)
                reject(err);
            else
                resolve();
...
async function loadStats(userId) {
    const row = await dbGet('SELECT wins, losses, draws, matches FROM tkm_scores WHERE user_id = ?', [userId]);
    return {
        wins: row?.wins || 0,
        losses: row?.losses || 0,
        draws: row?.draws || 0,
...
async function updateLeaderboard(challengerId, opponentId, result) {
...
function buildInviteButtons(inviteId, disabled = false) {
...
function buildMoveButtons(gameId, disabled = false) {
    return new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder()
...
function buildHistoryText(history, challengerTag, opponentTag) {
    if (history.length === 0)
        return 'Henuz tur oynanmadi.';
    return history
        .slice(-5)
        .map((h) => {
        let winnerText = 'Berabere';
        if (h.winner === 'challenger')
            winnerText = challengerTag;
        if (h.winner === 'opponent')
            winnerText = opponentTag;
...
function buildGameEmbed(game, title, footerText) {
    return new discord_js_1.EmbedBuilder()
        .setColor(0x111111)
        .setTitle(title)
        .setDescription(`**Oyuncular**\n` +
        `🔹 ${game.challengerTag}\n` +
        `🔸 ${game.opponentTag}\n\n` +
    ...
    async execute(interaction) {
        try {
            ...
            if (interaction.isButton()) {
                ...
                if (customId.startsWith('tkm_accept_')) {
                    ...
                    await interaction.update({
                        embeds: [
                            buildGameEmbed(game, '✂️ Tas Kagit Makas Basladi', 'Oyun basladi. Iki oyuncu da asagidan hamlesini secsin.'),
                        ],
                ...
                if (customId.startsWith('tkm_move_')) {
                    ...
                    if (game.challengerMove && game.opponentMove) {
                        ...
                        await gameMessage.edit({
                            embeds: [
                                buildGameEmbed(game, '🎮 Tas Kagit Makas Devam Ediyor', `Tur tamamlandi. Siradaki tur: **${game.currentRound}/${game.rounds}**`),
                            ],
                            components: [buildMoveButtons(game.id)],
                        });
                    }
            ...
            if (interaction.commandName === 'owner-bilgi') {
                ...
                const totalUsers = interaction.client.guilds.cache.reduce((sum, g) => sum + (g.memberCount || 0), 0);
                await interaction.reply({
            ...
            if (interaction.commandName === 'owner-duyuru') {
                ...
                for (const guild of interaction.client.guilds.cache.values()) {
                    const channel = guild.channels.cache.find((c) => c instanceof discord_js_1.TextChannel && c.name === channelName);
                    if (!channel)
                        continue;
                    try {
            ...
            if (interaction.commandName === 'tkm-skor') {
                ...
                const embed = new discord_js_1.EmbedBuilder()
                    ...
                    .addFields({ name: '🎮 Toplam Mac', value: `${stats.matches}`, inline: true }, { name: '✅ Galibiyet', value: `${stats.wins}`, inline: true }, { name: '❌ Maglubiyet', value: `${stats.losses}`, inline: true }, { name: '🤝 Beraberlik', value: `${stats.draws}`, inline: true }, { name: '📈 Kazanma Orani', value: `%${winRate}`, inline: true });
            ...
            if (interaction.commandName === 'taskagitmakas') {
                ...
                const inviteEmbed = new discord_js_1.EmbedBuilder()
                    ...
                    .setDescription(`${opponent} sen bir maca davet edildin.\n\n` +
            ...
            if (logCommands.includes(interaction.commandName)) {
                ...
                if (interaction.commandName === 'durum') {
                    ...
                    for (const name of names) {
                        const ch = await (0, channelManager_1.getLogChannel)(interaction.guild, name);
                ...
                if (interaction.commandName === 'kur-logs') {
                    ...
                    await (0, channelManager_1.createLogChannels)(interaction.guild);
                ...
                if (interaction.commandName in singleMap) {
                    ...
                    const channel = await (0, channelManager_1.createSingleLogChannel)(interaction.guild, channelName);
                    if (!channel) {
            ...
            if (needsModPerm.includes(interaction.commandName)) {
                const hasBan = interaction.memberPermissions?.has(discord_js_1.PermissionFlagsBits.BanMembers);
                const hasKick = interaction.memberPermissions?.has(discord_js_1.PermissionFlagsBits.KickMembers);
                const hasMute = interaction.memberPermissions?.has(discord_js_1.PermissionFlagsBits.ModerateMembers);
                const hasManage = interaction.memberPermissions?.has(discord_js_1.PermissionFlagsBits.ManageMessages);
                const hasAdmin = interaction.memberPermissions?.has(discord_js_1.PermissionFlagsBits.Administrator);
                let allowed = false;
